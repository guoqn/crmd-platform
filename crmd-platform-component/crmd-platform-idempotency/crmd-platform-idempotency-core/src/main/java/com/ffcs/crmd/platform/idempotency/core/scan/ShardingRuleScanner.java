package com.ffcs.crmd.platform.idempotency.core.scan;
...
public class ShardingRuleScanner extends ClassPathBeanDefinitionScanner {
    ...
    public ShardingRuleScanner(BeanDefinitionRegistry registry) {
            ...
            for (BeanDefinition candidate : candidates) {
                if (candidate instanceof GenericBeanDefinition) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("加载表分片规则:" + beanDefine.getBeanClassName() + "' ");
                    }
                    try {
                        Class<?> clazz = beanDefine.resolveBeanClass(this.getClass()
                            .getClassLoader());
                        if (clazz != null) {
                            ShardingRuleUtil.scanClazz(clazz);
                        }
                    } catch (Exception e1) {
                        e1.printStackTrace();
                    }
