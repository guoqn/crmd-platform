...
(function(angular) {
	...
	function selectedDataView(scope, elm){
		var group = elm.find("ul.list-group");
		if(group.length>0){
			var instance = elm.find("[js-tree]").jstree(true);
			var bottomNodes = instance.get_bottom_selected(true);
			var _html = "";
			$.each(bottomNodes,function(i, n){
				_html += ('<li class="list-group-item" id="_list_grout_item_'+ n.id +'" data="'+ n +'">'+ n.text +'</li>');
			});
			group.html(_html);
			return true;
		}
	...
	function inputViewData(scope, elm) {
		...
		elm.find("input").attr("title", _rtVal);
	}
