...
public class Endecrypt {
    ...
    private static class EndecryptHolder {
        static Endecrypt INSTANCE = new Endecrypt();
    }
    ...
    public static Endecrypt getInstance() {
        return EndecryptHolder.INSTANCE;
    }
...
//        System.out.println("进行3-DES解密后的内容: " + reValue2);
        ...
        if (args.length != 3) {
            throw new RuntimeException("args error,must 3 args");
        }

        if ("en".equals(args[0])) {
            System.out.println(getInstance().get3DESEncrypt(args[2],args[1]));
        } else if ("de".equals(args[0])) {
            System.out.println(getInstance().get3DESDecrypt(args[2],args[1]));
        } else {
            throw new RuntimeException("operType error." + args[0]);
        }
    }
}
