...
public class DynamicPageController extends AbstractController {
	...
	public RetVo getDynamicWinByEvents(@RequestBody DynamicPageVo dynamicPageVo) {
		RetVo retVo = new RetVo(true);
		DynamicWindowDTO windowDto = pageFacade.getDynamicWindowDto(
				StringUtils.isNullOrEmpty(dynamicPageVo.getBusType()) ? "" : dynamicPageVo.getBusType(),
				dynamicPageVo.getBusSpec() == null ? 0L : dynamicPageVo.getBusSpec(),
				dynamicPageVo.getBusObjId() == null ? 0L : dynamicPageVo.getBusObjId(),
				dynamicPageVo.getBusRegion() == null ? 0L : dynamicPageVo.getBusRegion(),
				dynamicPageVo.getBusService() == null ? 0L : dynamicPageVo.getBusService(),
				dynamicPageVo.getBusChannel() == null ? 0L : dynamicPageVo.getBusChannel());
		if (windowDto != null) {
			retVo.setObject(windowDto);
		}
		return retVo;
	 ...
	 * @return
	 */
	@RequestMapping("/queryPageInfoEvents")
	public RetVo queryPageInfoEvents(@RequestBody DynamicPageVo dynamicPageVo) {
		RetVo retVo = new RetVo(true);
		// String busType, Long busSpec, Long busObjId, Long busRegion,
		// Long busService, Long busChannel
		retVo.setPageInfo(pageFacade.queryPageInfoEvents(
				StringUtils.isNullOrEmpty(dynamicPageVo.getBusType()) ? "" : dynamicPageVo.getBusType(),
				dynamicPageVo.getBusSpec(), dynamicPageVo.getBusObjId(), dynamicPageVo.getBusRegion(),
				dynamicPageVo.getBusService(), dynamicPageVo.getBusChannel(),
				dynamicPageVo.getOffset() == null ? 0 : dynamicPageVo.getOffset(),
				dynamicPageVo.getLimit() == null ? 10 : dynamicPageVo.getLimit()));
		return retVo;
	}
